	...
	if (pnmlv->hdr.code == LVN_ITEMCHANGED)
	{
		if (pnmlv->uNewState & LVIS_FOCUSED)
		{
			...
			if(pProp == NULL)
				...
				m_wndDesc.SetWindowText(COLE2T(pProp->m_bstrDesc));
		}
	}
	...
	if ((pnmlv->hdr.code == NM_DBLCLK) || (pnmlv->hdr.code == NM_CLICK))
	{
		if (pnmlv->iSubItem == 1)
		{
			...
			if (pProp != NULL)
			{
				...
				{
					m_edit.SetWindowText(bstrText? COLE2T(bstrText) : _T(""));
					rc.top -= 2;
					rc.bottom -= 2;
...
LRESULT CPropertyBrowseControl::OnChar(UINT , WPARAM wParam, LPARAM , BOOL& )
{
	...
	if (ch == VK_RETURN)
	{
		...
		pProp->SetValue(COLE2T(bstrText));
		return 0;
	}
