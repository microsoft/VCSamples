...
void CPage3::OnFontType()
{
	...
	m_wndFont.SelectFont((LPCTSTR)m_strCurFontName, m_nCurFontCharSet);
}
...
void CPage3::OnSelchangeFont()
{
	CMFCFontInfo* pCurFontInfo = m_wndFont.GetSelFont();
	if (pCurFontInfo != NULL)
	{
		ASSERT_VALID(pCurFontInfo);

		m_strCurFontName = pCurFontInfo->m_strName;
		m_nCurFontCharSet = pCurFontInfo->m_nCharSet;
	}
}
