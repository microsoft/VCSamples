...
{
	std::auto_ptr<CEdit> apWndEdit(new CEdit ());

	apWndEdit->Create (WS_CHILD | WS_VISIBLE | WS_HSCROLL | WS_VSCROLL |
		ES_MULTILINE | ES_AUTOHSCROLL | ES_AUTOVSCROLL, 
		CRect (0, 0, 0, 0), &m_wndTabs, 1);
	apWndEdit->SetReadOnly ();
	apWndEdit->SetFont (m_Font.GetSafeHandle () == NULL ? &afxGlobalData.fontRegular : &m_Font);
	...
	if (nID != 0)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					apWndEdit->SetWindowText(lpszText);
					delete[] lpszText;
				}
	...
	m_wndTabs.AddTab (apWndEdit.release(), lpszName, (UINT)-1, FALSE);
}
