...
#define UM_RECALCLAYOUT (WM_USER + 101)
...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWnd)
	...
	ON_MESSAGE(UM_RECALCLAYOUT, OnPostRecalcLayout)
...
void CMainFrame::UpdateMDITabs (BOOL bResetMDIChild)
{
	...
	case CMDITabOptions::MDITabsStandard:
		...
		EnableMDITabs (TRUE,theApp.m_Options.m_bMDITabsIcons, theApp.m_Options.m_bTabsOnTop ? CMFCTabCtrl::LOCATION_TOP : CMFCTabCtrl::LOCATION_BOTTOM, TRUE, theApp.m_Options.m_nTabsStyle);
    ...
    PostMessage(UM_RECALCLAYOUT);
}

LRESULT CMainFrame::OnPostRecalcLayout(WPARAM, LPARAM)
{
	RecalcLayout ();
	RedrawWindow (NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
...
void CMainFrame::OnSysColorChange() 
{
	...
	if (theApp.m_Options.m_nMDITabsType == CMDITabOptions::None)
	{
		UpdateMDITabs(TRUE);
	}
}
