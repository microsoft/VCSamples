...
SOURCE=.\AutoServ.cpp
"$(OUTDIR)\AutoServ.obj"	"$(OUTDIR)\$(PREHEADER)" : $(SOURCE) "$(OUTDIR)"
	$(CPP) @<<
  $(CPP_SWITCHES) $(SOURCE)
<<


SOURCE=.\AutoServer.cpp
"$(OUTDIR)\AutoServer.obj" : $(SOURCE) "$(OUTDIR)" "$(OUTDIR)\$(PREHEADER)"
	$(CPP) @<<
   $(CPP_PROJ) $(SOURCE)
<<
...
LINK32_OBJS= \
	"$(OUTDIR)\AutoServ.obj" \
	"$(OUTDIR)\AutoServer.res" \
	"$(OUTDIR)\AutoServer.obj" \
	"$(OUTDIR)\StdAfx.obj" 
